...
craft = sphere(pos=vector(-10*Earth.radius, 0,0),radius = 1.5e6, color=color.yellow)
...
KUgraph = gdisplay(title = 'K, U, and K + U vs. t', x=0, y=0, width=500, height=200)
...
Wgraph = gdisplay( title = 'Cumulative Work done on craft',x=500, y=0, width=500, height=200 )
Wcurve = gcurve( color = color.green )
...
while t < t_f:
    ...
    PEarth = -((G* mCraft * mEarth) / mag( rEarthCraft))
    PMoon = -((G* mCraft * mMoon)/ mag(rMoonCraft))
    K = (0.5*((mag2(pCraft)/mCraft))) ## you must complete this line
    U = PEarth + PMoon  ## you must complete this; omit the constant term for the Earth-Moon
    
    
    # Calculate cumulative work done internally by the system.
    W = W + dot(FEarth, delta_r) + dot(FMoon, delta_r) # you must complete this line.
